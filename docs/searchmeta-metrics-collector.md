...
Computing an aggregate such as "average reading value for category 0" over a search
result set was executed as
...
Creating a regular MongoDB index per aggregation is not a general answer: the
combination of filters and aggregated fields is open-ended (ad-hoc analytics, dashboards,
user- or machine-generated queries), and the search index already knows which documents
match.
...
- **Per-facet-bucket metrics** ("avg value per category") - the most valuable next
...
- **Doc-visit cap** - a broad filter visits every matching document, like
  `count: {type: "total"}` today. Consider a limit if untrusted or generated queries
